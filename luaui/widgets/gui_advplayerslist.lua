...
-- v17   (Jools): made to work with xta and fix some bugs, like the invincible ai team bug.
...
sidePics['ai'] 		  = imageDirectory..'imac_small.dds'
...
function SetMaxPlayerNameWidth()
	...
	local maxWidth = 14*gl_GetTextWidth(absentName2) + 8 -- 8 is minimal width
	local name = ""
	local nextWidth = 0
...
function CreatePlayerFromTeam(teamID) -- for when we don't have a human player occupying the slot, also when a player changes team (dies)
	...
	if isAI then
		local skirmishAIID,name,host,shortName
		skirmishAIID,name,host,shortName, tname = Spring_GetAIInfo(teamID)
		tdead = IsAIDead(teamID)
		
		if tdead then
			tname = absentName5
		else
			local remoteName = Spring.GetGameRulesParam("AI-Name"..teamID) or "(remote)"
			if (not shortName) or (shortName == "") or shortName:find("KNOWN") then
				tname = "AI: " .. remoteName 
			else
				tname = "AI: " .. (shortName or "?")
			end
		end
...
function DrawPlayer(playerID, leader, vOffset, mouseX, mouseY)
	...
	if spec == false then --player
		...
		if m_name.active == true then
			DrawName(name, team, posY, dark, playerID, dead)
		end
		if ai then -- ai
...
function DrawSidePic(team, playerID, posY, leader, dark, ai)
	...
	if gameStarted then
		...
		DrawState(playerID, m_side.posX + widgetPosX, posY,ai)
	end
	
	if ai then
		gl_Texture(sidePics['ai'])
		DrawRect(m_rank.posX + widgetPosX + 1, posY+17, m_rank.posX + widgetPosX + 17, posY + 1) -- draws side image
		gl_Texture(false)
	end
...
function DrawState(playerID, posX, posY, ai)
	...
	local aicomputing = (playerReadyState[playerID]==-1) --FIXME, not getting this value
		
	if ai then
		if aicomputing then
			gl_Color(0.5, 0.5, 0.5, 0.8) -- grey
		else
...
function DrawName(name, team, posY, dark, playerID,dead)
    local willSub = ""
	...
	if (nameColourR + nameColourG*1.35 + nameColourB*0.5) < 0.8 then
		...
		gl_Color(nameColourR,nameColourG,nameColourB,alpha)
			...
			if mySpecStatus == true then
				...
				if i>-1 and i<64 then
					if m_name.active and clickedPlayer.name ~= absentName and IsOnRect(x, y, m_name.posX + widgetPosX +1, posY, m_name.posX + widgetPosX + m_name.width, posY+16) then
						if ctrl then 
							Spring.SendCommands{"toggleignore "..clickedPlayer.name} 
							return true 
						end
						if clickTime - prevClickTime < dblclickPeriod and clickedName == prevClickedName then 
							...
							if not clickedPlayer.spec then 
								Spring.SendCommands{"specteam "..i}
							end
							SortList()
							CreateLists()
							return true
								...
								if release ~= nil then                                                                                --
									if release >= now then                                                                            --
										if clickedPlayer.team == myTeamID then                                                        --
											Spring.SendCommands("say a: I need unit support!")                                        -- (ask)
										else                                                                                          --
											local suc = Spring.GetSelectedUnitsCount()
											Spring.SendCommands("say a: I gave "..suc.." units to "..clickedPlayer.name..".")
					...
					if i > -1 and i < 64 then
						if m_chat.active == true then
							if IsOnRect(x, y, m_chat.posX + widgetPosX +1, posY, m_chat.posX + widgetPosX +17,posY+16) then                            --chat button
								Spring.SendCommands("chatall","pastetext /w "..clickedPlayer.name..' \1')
								return true                                                                                                                --
							end
						end
						if m_point.active == true then
						...
						if m_name.active and clickedPlayer.name ~= absentName and IsOnRect(x, y, m_name.posX + widgetPosX +1, posY, m_name.posX + widgetPosX + m_name.width, posY+12)  then
							...
							if clickTime - prevClickTime < dblclickPeriod and clickedName == prevClickedName then 
								...
								prevClickedName = ''
								if not clickedPlayer.spec then
									Spring.SendCommands{"specteam "..clickedPlayer.team}
								end
								SortList()
								CreateLists()
								return true
...
function widget:MouseRelease(x,y,button)
	if button == 1 then
		...
		if energyPlayer ~= nil then                                                -- share energy/metal mouse release
			if energyPlayer.team == myTeamID then
				if amountEM == 0 then
					Spring.SendCommands("say a: I need Energy!")
				else
					Spring.SendCommands("say a: I need "..amountEM.." Energy!")
				end
			elseif amountEM > 0 then
				Spring_ShareResources(energyPlayer.team, "energy", amountEM)
				Spring.SendCommands("say a: I sent "..amountEM.." energy to "..energyPlayer.name)
			end
			sliderOrigin = nil
			amountEMMax = nil
			sliderPosition = nil
		...
		if metalPlayer ~= nil then
			if metalPlayer.team == myTeamID then
				if amountEM == 0 then
					Spring.SendCommands("say a: I need Metal!")
				else
					Spring.SendCommands("say a: I need "..amountEM.." Metal!")
				end
			elseif amountEM > 0 then
				Spring_ShareResources(metalPlayer.team, "metal", amountEM)
				Spring.SendCommands("say a: I sent "..amountEM.." metal to "..metalPlayer.name)
			end
			sliderOrigin = nil
			amountEMMax = nil
			sliderPosition = nil
...
function Take(teamID,name, i)
	...
	Spring.SendCommands("luarules take2 " .. teamID)
	return
end
