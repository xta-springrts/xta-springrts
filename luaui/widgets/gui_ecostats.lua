...
local images			= {
						...
						["talon"]				= "LuaUI/Images/ecostats/talon_default.png",
...
local function friendlyName(teamID)
	...
	if isAIteam then
		...
		elseif side == "talon" then return "Talon"
...
local function DrawExpandTableBorders(allyID)
	if not (allyData[allyID+1].exists) then return end
	
	local x1, x2, y1, y2
	local w = 180 + cW* #(allyData[allyID+1].teams) -- infotable width
	x1 = iPosX[allyID]
	y2 = iPosY[allyID]
	x2 = x1 + w
	...
	if Button["info"][allyID].click then
		glColor(0,0,0,1)	-- highlight borders
	else
...
local function DrawOptionRibbon()
	...
	if Options["borders"]["On"] then
		glTexture(images["borderson"])
	else
		glTexture(images["bordersoff"])
	end
	glTexRect(
		Options["borders"]["x1"],
		Options["borders"]["y1"],
...
function widget:IsAbove(x, y)
	...
	local speed,_,paused = GetGameSpeed()
		
	for name, buttonType in pairs(Button) do
		
		if name == "player" or name == "info" then
			for id,button in pairs(buttonType) do
				button.mouse = false
				if IsOnButton(x,y, button.x1,button.y1,button.x2,button.y2) then
					button.mouse = true
