...
function Scale:DrawControl()
  ...
  if (self.min <=0 and self.max >= 0) then 
		local scale = self.width / (self.max-self.min)
		fh.DrawCentered(0, self.x +  scale * (0 - self.min), self.y, self.fontsize,'a')
  end 
  fh.Draw(self.min, self.x, self.y, self.fontsize,'a')
  fh.DrawRight("+"..self.max, self.x+self.width, self.y, self.fontsize,'a')
