...
--Changelog
--1.24: Compatibility fix
--1.22:
--fixed uninitialized saveQueue-table on first start
--
--1.21:
--added: Press Meta+C to clear currently selected factories queue
--added: some speedups, but its still quite hungry while displaying menu
...
function DrawTexRect(left,top,right,bottom, texture, alpha)
	...
	glTexRect(left,bottom,right,top)
	glColor(1,1,1,1)
	glTexture(false)
end
...
function DrawBoxTitle(x,y,alpha, unitDef, selUnit)
  --local x,y,z = CalcDrawCoords( selUnit )
	...
	glColor(0,0,0, math.min(alpha,0.5) )
	glRect( x,y, x + boxWidth,y - boxHeightTitle )
	glColor(1,1,1,1)
	...
	glColor(0,1,0,alpha or 1)
	glText( text, x + boxHeightTitle + titleTextXOff, y - boxHeightTitle/2.0 - titleTextYOff, fontSizeTitle, "n")
	glColor(1,1,1,1)
end
...
function DrawBoxGroup( x, y, yOffset, unitDef, selUnit, alpha, groupNo, queue )
	...
	if ( ( modifiedGroup == groupNo ) and ( modifiedGroupTime > Spring.GetGameSeconds() - loadedBorderDisplayTime ) ) then
		if ( modifiedSaved == true ) then
			glColor(1,0,0, math.min(alpha,1.0) )
		else
			glColor(0,1,0, math.min(alpha,1.0) )
  	end
  	glRect( x - loadedBorderWidth ,y + loadedBorderWidth, x + boxWidth + loadedBorderWidth,y - boxHeight - loadedBorderWidth)
	end

	--Draw Background Box
	glColor(0,0,0, math.min(alpha,0.6) )
	glRect( x,y, x + boxWidth,y - boxHeight )
	if ( ( queue[facRepeatIdx] == nil ) or ( queue[facRepeatIdx] == true ) ) then
		glColor( 0.0,  0.7, 0.0, math.min( alpha or 1, 0.5 ) )
	else
		glColor( 0.7,  0.7, 0.7, math.min( alpha or 1, 0.5 ) )
	end
	glRect( x + boxIconBorder ,y - 3, x + groupLabelMargin,y - boxHeight + 3  )
	
	--Draw group Label
	text = groupNo
	glColor( 1.0, 0.5, 0,alpha or 1)
	glText( text, x + groupLabelXOff, y - boxHeight/2.0 - groupLabelYOff, fontSizeGroup, "cn")
	...
	for k,unitCount in pairs( units ) do
		if ( ( x + boxHeight + boxIconBorder + xOff + boxHeight + unitIconSpacing) >  x + boxWidth ) then
			glColor(1,1,1,alpha)
			glText( "...", x + xOff + unitCountXOff, y - boxHeight + unitCountYOff, fontSizeUnitCount, "n")
			break
		else
			DrawTexRect( x + boxIconBorder + xOff,y - boxIconBorder,x + boxHeight + boxIconBorder + xOff,y - boxHeight +  boxIconBorder,"#" .. k, alpha )
			glColor(1,1,1,alpha)
			glText( unitCount, x + xOff + unitCountXOff, y - boxHeight + unitCountYOff, fontSizeUnitCount, "cn")
		end
	...
	if ( ( modifiedGroup == groupNo ) and ( modifiedGroupTime > Spring.GetGameSeconds() - loadedBorderDisplayTime ) ) then
		...
		glColor(0.9,0.9,0.9, alpha )
 		glText( lText, x + (boxWidth + 0.5)/2, y - (boxHeight + 0.5)/2 - fontModifiedYOff, fontSizeModifed, "cn")
	end
