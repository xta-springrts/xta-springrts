...
function gadget:UnitCreated(uID, uDefID, uTeam)
    if canPassive[uDefID] then
		if (spFindUnitCmdDesc(uID, CMD_PASSIVE)) then
			return  -- already exists
		end
		local insertID = 
			spFindUnitCmdDesc(uID, CMD.CLOAK)      or
			spFindUnitCmdDesc(uID, CMD.ONOFF)      or
			spFindUnitCmdDesc(uID, CMD.TRAJECTORY) or
			spFindUnitCmdDesc(uID, CMD.REPEAT)     or
			spFindUnitCmdDesc(uID, CMD.MOVE_STATE) or
			spFindUnitCmdDesc(uID, CMD.FIRE_STATE) or
			spFindUnitCmdDesc(uID, CMD.GUARD) or
			123456 -- back of the pack
	
        spInsertUnitCmdDesc(uID, insertID + 1, cmdPassiveDesc)
    end
end
...
function gadget:AllowCommand(uID, uDefID, uTeam, cmdID, cmdParams, cmdOptions, cmdTag, synced)
    if cmdID == CMD_PASSIVE and canPassive[uDefID] then
        ...
        passiveBuilders[uID] = (cmdParams[1] == 0)
