...
local COMMANDER = {
  [UnitDefNames["core_ucommander"].id] = true,
  ...
  for playerID, defs in pairs(startPoints) do
	...
	if modoptioncomtype and modoptioncomtype ~="zeroupgrade" then
		if comtype == "arm_u0commander" then
			if modoptioncomtype == "halfupgrade" then
				comtype = "arm_u2commander"
			elseif modoptioncomtype == "fullupgrade" then
			...
			elseif modoptioncomtype == "capturethebase" then
				comtype = "arm_base"
			elseif modoptioncomtype == "nincom" then
				comtype = "arm_nincommander"
			end
		elseif comtype == "core_ucommander" then
			if modoptioncomtype == "halfupgrade" then
				comtype = "core_u2commander"
			elseif modoptioncomtype == "fullupgrade" then
			...
			elseif modoptioncomtype == "capturethebase" then
				comtype = "core_base"
			elseif modoptioncomtype == "nincom" then
				comtype = "core_nincommander"
			end
		end
	end


    local commanderID = Spring.CreateUnit(comtype, defs.x, defs.y, defs.z, 1, defs.teamID)
    if nudge then
  ...
  if preStart and startPoints[id] then
    ...
    elseif (msg:find("coop_factionchange",1,true)) then
      ...
      if id == (playerID * 1) then
        if startPoints[id].commander == "arm_u0commander" then
          startPoints[id].commander = "core_ucommander"
        else
          startPoints[id].commander = "arm_u0commander"
        end
...
function StartPoint(_,x,z,name,playerID,faction)
  if (startPoints[playerID].teamID == myTeamID) or isspec or AreTeamsAllied(startPoints[playerID].teamID,myTeamID) then
    ...
    if faction == "arm_u0commander" then
