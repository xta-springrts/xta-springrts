...
local function RawCreateUnit(...)
  ...
  UnitCreated = nil
  local unitID = Spring.CreateUnit(unpack(arg))
...
local function RawGiveOrderToUnit(...)
  ...
  AllowCommand = nil
  Spring.GiveOrderToUnit(unpack(arg))
...
local function AddBuildCmdDesc(unitID, buildDefID)
  ...
  Spring.InsertUnitCmdDesc(unitID, {
    ...
    name   = ud.name,
    action = 'buildunit_' .. string.lower(ud.name),
...
local function StartGame()
  ...
  for id,team in pairs(teams) do
    ...
    Spring.SetTeamResource(team.id, 'metal',  team.metalStorage)
    Spring.SetTeamResource(team.id, 'energy', team.energyStorage)
  end
...
local function GetCancelBuildID(teamID, buildID, cmdParams)
  ...
  local btwist = (math.mod(bfacing, 2) == 1)
  ...
  for _,uid in ipairs(units) do
    ...
    local twist = (math.mod(facing, 2) == 1)
...
function GotChatMsg(msg, playerID)
  ...
  if ((cmd == 'run') or (cmd == 'echo')) then
    ...
    local _,_,line = string.find(msg, "[%s]*[^%s]+[%s]+(.*)")
    if (cmd == 'run') then
